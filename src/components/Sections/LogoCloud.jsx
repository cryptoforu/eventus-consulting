    ...
    >
      <Container>
        ...
        >
          <Heading
            headingLevel="h2"
            className={"font-bold text-5xl sm:text-6xl md:text-7xl text-center"}
          >
            ...
            {partners.map(({ node }, index) => (
              <Link key={node.id} to={node.link}>
                <motion.div
                  ...
                  className="relative group aspect-[500/260] rounded-xl border border-slate-200 py-2 px-4 bg-white bg-clip-padding bg-opacity-50 backdrop-filter backdrop-blur-[5px] dark:backdrop-blur-none dark:bg-slate-950 dark:border-gray-900"
                >
                  {isHoveredI === index && (
                    <motion.div
                      layoutId="underline"
